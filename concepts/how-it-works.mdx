---
title: "How Nebula Works: MCP, Hub, and Stellar"
...
- **Privy custody** — the Hub holds your wallet's private key inside Privy's embedded wallet infrastructure. Signing happens here, server-side. The key never travels over the network.
...
<Steps>
  ...
  <Step title="Outcome is acted on">
    - **`auto`** — the Hub calls Privy to sign the Stellar transaction XDR and broadcasts it. The tool returns `status: "ok"` with the transaction hash.
...
1. Creates a Privy embedded wallet on Stellar (testnet or mainnet, depending on your account setting).
